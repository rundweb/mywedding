...
import PrimaryLayouts from "./PrimaryLayouts";
...
const FirstLayouts = () => {
  ...
  return (
    <div className="relative max-w-lg m-auto min-h-screen overflow-hidden">
      <div
        className={`fixed inset-0 max-w-lg m-auto flex items-center justify-center min-h-screen bg-cover bg-center z-50 transition-all duration-1000 ease-in-out ${
          openPages ? "-translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
        }`}
      ...
      >
        <div>
          <div className="relative -top-10">
            ...
            <div className="absolute -bottom-52 z-40 left-2/4 -translate-2/4 w-max flex flex-col items-center justify-center">
              ...
              <button
                onClick={ButtonOpen}
                className="cursor-pointer mt-4 z-50 bg-primary-maroon text-light-blush py-2.5 px-6 shadow-xl rounded-full font-Poppins font-medium tracking-wide w-max uppercase"
              >
                open invitation
              </button>
            </div>
      ...
      <div
        className={`transition-opacity duration-1000 ${openPages ? "opacity-100" : "opacity-0"}`}
      >
        <PrimaryLayouts />
      </div>
    </div>
